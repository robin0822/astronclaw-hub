...
实现阶段采用 TDD 约束，不允许先完成代码后再补只验证实现细节的测试。Agent 必须从本方案的验收标准、状态机、接口契约、权限、审计、错误码和数据来源要求中抽取行为测试，再实现生产代码。

本仓库将文章中的实现阶段机制落为以下工程卡口：

- `code-execute` 负责在已确认 `requirements.md` 和 `plan.md` 范围内推进 TDD 实现。
- `cr-collaboration` 负责内部结构化 review 和 CR 闭环。
- `.githooks/pre-push` 在 `git push` 时自动触发质量门禁，失败直接拒绝 push。
- `scripts/quality-gate.sh` 是统一门禁入口。
- `scripts/diff-to-test-map.cjs` 检查生产代码变更是否有具体测试锚点。
- `scripts/agent-code-review.cjs` 对待 push diff 做内部结构化 review。
- `quality/test-map.json` 维护稳定的生产代码到测试锚点映射。

质量门禁包含：

- diff-to-test 映射。
- 后端 `ruff`、前端 `oxlint/typecheck` 等静态规则检查。
- 后端 `pytest --cov` 覆盖率检查。
- Agent 内部结构化 review。
